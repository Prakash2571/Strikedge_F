...
export function BoxOperationalState({ status }: { status: BoxStatus | undefined | null }) {
  // Render only when the backend actually published the v1.5.0 operational block. A missing field
  ...
  return (
    <section className="box-exec-health" aria-label="Operational state">
      ...
      {/* ── HEADLINE ENTRY GATE — honesty rule #2 made visible ─────────────────────────────── */}
  ...
  if (gate.entryPermitted) {
    return (
      <p className="box-op-gate box-op-gate--ok" role="status">
        <span aria-hidden="true">{bandGlyph("ready")}</span> New entry is permitted — both
        market-data and order-update transports are ready.
      </p>
    );
  }
  ...
  return (
    ...
    >
      <p className="box-op-gate-head">
        ...
        {broken ? (
          <strong>Feed is broken — new entry stopped.</strong>
        ) : (
      ...
      <p className="box-exec-note">
        {gate.positionsManageable
          ? "Existing positions can still be exited, reduced and protectively cancelled."
          : "The session is expired — the broker will refuse everything but a protective cancel."}
      </p>
    </div>
  );
}
